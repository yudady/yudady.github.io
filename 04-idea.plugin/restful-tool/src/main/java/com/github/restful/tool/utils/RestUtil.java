...
public class RestUtil {
    ...
    public static boolean hasRestful(@Nullable PsiClass psiClass) {
        ...
        return SpringHelper.hasRestful(psiClass) || JaxrsHelper.hasRestful(psiClass) || UnistarHelper.hasRestful(psiClass);
    }
    ...
    private static String getPomFileProperties(@Nullable Element element, String name) {
        ...
        @Language("RegExp")
        final String propReg = "\\$\\{[A-Za-z0-9.:-]+}";
        if (name == null) {
            return "";
        }
        if (element == null) {
    ...
    public static List<Request> getCurrClassRequests(@Nullable PsiClass psiClass) {
        ...
        if (!(requests = JaxrsHelper.getCurrClassRequests(psiClass)).isEmpty()) {
            return requests;
        }

        if (!(requests = UnistarHelper.getCurrClassRequests(psiClass)).isEmpty()) {
            return requests;
        }
