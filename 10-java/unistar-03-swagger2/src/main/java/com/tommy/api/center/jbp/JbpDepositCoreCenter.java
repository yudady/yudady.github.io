...
import com.tommy.api.center.jbp.model.JbpSendDepositResponse;
...
@RestController
@RequestMapping(value = "/jbp-deposit", produces = APPLICATION_JSON_VALUE) //配置返回值 application/json
@Api(tags = "jbp-deposit-直接呼叫core-center")
public class JbpDepositCoreCenter {
    ...
    @ApiOperation("充值申請")
    @PostMapping("/send")
    public ResponseEntity<JbpSendDepositResponse> send(
        @ApiParam(value = "JbpSendDepositRequest", required = true)
        @RequestBody final JbpSendDepositRequest jbpSendDepositRequest) {
        final String path = "/jbp-deposit/send";
        ...
        // -----------------------------------------
        // build the request
        final HttpEntity<JbpSendDepositRequest> request = new HttpEntity<>(jbpSendDepositRequest, headers);
        // send POST request
        return restTemplate.postForEntity(uri, request, JbpSendDepositResponse.class);

    }


    private void bindData(final JbpSendDepositRequest jbpSendDepositRequest) {
        jbpSendDepositRequest.channelId = 1L;
        jbpSendDepositRequest.companyOrderNum = "U5" + MD5.md5Hex("UB" + System.currentTimeMillis() + "DA");
    }

    @ApiOperation("充值申請key")
    @PostMapping("/key")
    public String testKey(final JbpSendDepositRequest jbpSendDepositRequest) {
        ...
        return MD5.md5Hex(depositEncryptSource);
    }
