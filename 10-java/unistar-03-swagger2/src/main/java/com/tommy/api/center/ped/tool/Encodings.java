...
public final class Encodings {
    ...
    public static String uriComponent(final String value) {
    ...
    public static String decodeURIComponent(final String value) {
        final int length = value.length();
        int index = 0;
        for (; index < length; index++) {
            final int ch = value.charAt(index);
            if (ch == '%') break;
        }
        ...
        for (; index < length; index++) {
            ...
            if (ch == '%') {
                ...
                final char hex1 = value.charAt(index + 1);
                final char hex2 = value.charAt(index + 2);
    ...
    public static String base64(final byte[] value) {
    ...
    public static String base64URLSafe(final byte[] value) {
        return Base64.getUrlEncoder().encodeToString(value);
    }

    public static byte[] decodeBase64URLSafe(final String value) {
        return Base64.getUrlDecoder().decode(value);
    }

    public static String hex(final byte[] bytes) {
        final byte[] values = new byte[bytes.length * 2];
        int i = 0;
